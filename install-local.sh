#!/bin/bash
#
# NOFX Nado Edition - Local Installation Script
# This script uses the local code with Nado integration
...
echo -e "${BLUE}"
echo "╔════════════════════════════════════════════════════════════╗"
echo "║            NOFX AI Trading OS - NADO EDITION               ║"
echo "║              Local Installation (with Nado)               ║"
echo "║                                                              ║"
echo "║           ✨ Includes Nado DEX Integration ✨              ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
if [ ! -f "$COMPOSE_FILE" ]; then
    echo -e "${YELLOW}⚠️  $COMPOSE_FILE not found in current directory${NC}"
    echo -e "${YELLOW}   Looking for docker-compose.prod.yml in: $INSTALL_DIR${NC}"
    echo ""
    echo -e "${BLUE}Available files:${NC}"
    ls -1 | grep -E "(docker-compose|package\.json)" || echo "  (No docker files found)"
    echo ""

    # Try to find docker-compose files
    if [ -f "docker-compose.yml" ]; then
        COMPOSE_FILE="docker-compose.yml"
        echo -e "${GREEN}✓ Using docker-compose.yml instead${NC}"
    else
        echo -e "${RED}Error: No docker-compose file found${NC}"
        echo "Please run this script from the NOFX root directory"
        exit 1
    fi
fi
...
print_success() {
    ...
    echo ""
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗"
    echo -e "║          ✨ Nado DEX Integration Activated ✨            ║"
    echo -e "╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo "  • Built on Ink Layer2"
    echo "  • Get started: https://www.nado.xyz/"
    echo ""
    echo -e "${YELLOW}Available Exchanges:${NC}"
    echo "  • Binance Futures (CEX)"
    echo "  • Bybit Futures (CEX)"
    echo "  • OKX Futures (CEX)"
    echo "  • Bitget Futures (CEX)"
    echo "  • Hyperliquid (DEX)"
    echo "  • Aster DEX (DEX)"
    echo "  • Lighter (DEX)"
    echo "  • ${GREEN}Nado (DEX) 🆕${NC}"
    echo ""
    ...
    echo "  3. ${GREEN}Configure Exchanges including Nado!${NC}"
